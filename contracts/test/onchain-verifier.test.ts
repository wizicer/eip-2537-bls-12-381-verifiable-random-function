...
describe('On-Chain VRF Verifier', () => {
  ...
  before(async () => {
    ...
    verifier = await ethers.deployContract("contracts/verifier.sol:EIP2537VrfStyleVerifier");
  });
  ...
  it('should verify VRF proof generated by JavaScript prover', async () => {
    ...
    // 设置足够的 gas limit，因为 verify 内部会调用 G1MSM 预编译（需要 ~50000 gas）
    // 加上其他操作（MAP_FP_TO_G1, keccak256等），总 gas 需求较高
    // Note: verify() 不再是 view 函数，需要使用 call 而不是 staticCall
    // verify 返回 (bool ok, bytes32 out)，需要正确解构
    let result: any;
    try {
      ...
      // 使用 staticCall 获取返回值
      const [ok, out] = await verifier.verify.staticCall(
        pkUncompressed,
        inputBytes,
        proof.c,
        proof.s_1,
  ...
  it('should reject invalid proof', async () => {
    ...
    // 调用合约：先发送交易，再使用 staticCall 获取返回值
    const tx2 = await verifier.verify(
      pkUncompressed,
      inputBytes,
      tamperedC,
      proof.s_1,
    ...
    const [ok2, out2] = await verifier.verify.staticCall(
      pkUncompressed,
      inputBytes,
      tamperedC,
      proof.s_1,
    ...
    expect(out2).to.equal(ethers.ZeroHash);
  });
